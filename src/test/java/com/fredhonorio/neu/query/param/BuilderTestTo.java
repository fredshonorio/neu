...
public class BuilderTestTo {
    ...
    public static Ref REF = Ref.of("r");
    ...
    public void _10x() {
        testQuery(
            b -> b.Match().node().to(REF).node().Return("x"),
            "MATCH () -[r]-> () RETURN x"
        );

        testQuery(
            b -> b.Match().node().to(REF, PROP).node().Return("x"),
    ...
    public void _11x() {
        testQuery(
            b -> b.Match().node().to(REF, TYPE).node().Return("x"),
            "MATCH () -[r:T]-> () RETURN x"
        );

        testQuery(
            b -> b.Match().node().to(REF, TYPE, PROP).node().Return("x"),
